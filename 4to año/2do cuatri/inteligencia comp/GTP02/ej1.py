...
while epoca < max_epocas and ecm > tol:
    ...
    for p in range(cant_patrones_trn):
        ...
        for c in range(cant_capas - 1):
            ...
            net = np.dot(pesos[c], entrada_con_bias)
            sal = sigmoide(net)
            netas.append(net)
            salidas.append(sal)
        ...
        deltas[-1] = error * sigmoide_derivada(netas[-1])
        ...
        for c in range(cant_capas - 3, -1, -1):
            ...
            deltas[c] = sigmoide_derivada(netas[c]) * np.dot(pesos[c+1][:, 1:].T, deltas[c+1])
