function [c_Line,c_Poly,c_Cir,c_Arc,c_Poi] = f_LectDxf(nomArch,cirXData)
   ...
   indInSecEnt = find(strcmp('ENTITIES',c_ValAsoc(m_PosCero(1:end-1)+1)));
   %(0,ENDSEC)
   m_indFinSecEnt = find(strcmp('ENDSEC',c_ValAsoc(m_PosCero(indInSecEnt:end))));
   % Entities Position
   ...
   for iEnt = 1:length(m_PosCero)-2
      ...
      switch nomEnt
         case 'LINE'
            ...
            % Thickness
            c_Line{iLine,3} = str2double(f_ValGrCode(39,m_GrCodeEnt,c_ValAsocEnt));
            % Add properties
            %
            iLine = iLine+1;
         case 'LWPOLYLINE'
         ...
         case 'CIRCLE'
            ...
            % XData
            if exist('cirXData','var')
               %Lo que se quiere leer es un número entero.
               c_Cir(iCir,3) = f_XData(1070,cirXData,m_GrCodeEnt,c_ValAsocEnt);
            end
            %
            iCir = iCir+1;
         case 'ARC'
...
function c_XData = f_XData(grCode,XDatNom,m_GrCode,c_ValAsoc)
   ...
   if ~isempty(m_PosXData)
      ...
      %Se hace case insensitive, aunque la aplicación del xdata es case sensitive.
      indInXData = m_PosXData(strcmpi(XDatNom,c_ValAsoc(m_PosXData)));
